...
- Nous commençons par vérifier la position de l’instruction d’emprunt pour nous assurer qu’elle est la seule
- Nous vérifions ensuite que le programme n’est pas invoqué via CPI afin de garantir qu’aucun emprunt multiple n’est effectué
- Puis, nous vérifions le nombre d’instructions et nous nous assurons que nous chargeons la dernière instruction de la transaction
- Ensuite, nous confirmons qu’il s’agit bien de l’instruction de remboursement en vérifiant l’ID du programme et le discriminateur de l’instruction
- Enfin, nous vérifions que les ATA transmis dans l’instruction de remboursement sont les mêmes que ceux que nous passons dans notre instruction `Borrow`
