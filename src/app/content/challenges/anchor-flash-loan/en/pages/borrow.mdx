...
- We then check that the program is not being invoked through CPI to ensure that multiple borrows aren't made
...
let current_instruction = load_instruction_at_checked(0, &ix_sysvar).unwrap();
