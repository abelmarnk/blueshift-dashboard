...
On a final note, notice we did not make any checks for multiple borrow instructions, or borrowing through CPI, which would have allowed for unrepaid borrows, this is because of the loan data account which would allow for only one borrow instruction(Can you tell why?), so those checks aren't necessary, though this approach would make transactions executable in parallel across fee tiers rather than across pools.
